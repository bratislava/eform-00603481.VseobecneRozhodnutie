...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="egonp:GeneralAgenda">
        <xsl:text>Všeobecné rozhodnutie</xsl:text>
<xsl:apply-templates />
    </xsl:template>
    <xsl:template match="egonp:GeneralAgenda/egonp:subject">
        <xsl:if test="./text()">
            <xsl:text>&#xA;</xsl:text>
<xsl:text>&#09;</xsl:text><xsl:text>Predmet: </xsl:text><xsl:call-template
    ...
    <xsl:template match="egonp:GeneralAgenda/egonp:text">
        <xsl:if test="./text()">
            <xsl:text>&#xA;</xsl:text>
<xsl:text>&#09;</xsl:text><xsl:text>Text: </xsl:text><xsl:call-template
                name="string-replace-all">
                <xsl:with-param name="text" select="." />
                <xsl:with-param name="replace" select="'%0A'" />
                <xsl:with-param name="by" select="'&#13;&#10;&#09;'" />
...
<xsl:variable name="boolValueString"
            select="string($boolValue)" />
<xsl:choose>
